...
public class Select implements Builder {
    ...
    /*
     * public Select(Database db, String columns, String tables) throws DatabaseException { //setDatabase(db); setColumns(columns); setTables(tables);
     * }
     */
    ...
    public Select update(Boolean update) {
        /*
         * if (update == null) { this.update = null; return this; }
         */
        this.update = update;
        return this;
    }
    ...
    /*
     * private void setColumns(String columns) throws DatabaseException { if (columns == null || columns.isEmpty()) throw new
     * DatabaseException("String columns cannot be null or empty in SELECT query.");
     * 
     * if (columns.contains(",")) { String[] temp = columns.split(",");
     * 
     * for (String t : temp) if (t.split("`", -1).length - 1 == 2) throw new
     * DatabaseException("Invalid syntax in SELECT query. Two backticks must surround column " + t + ".");
     * 
     * this.columns = Arrays.asList(temp); } else { if (columns.split("`", -1).length - 1 == 2) throw new
     * DatabaseException("Invalid syntax in SELECT query. Two backticks must surround column " + columns + ".");
     * 
     * this.columns.set(0, columns); } }
     * 
     * private void setTables(String tables) throws DatabaseException { if (tables == null || tables.isEmpty()) throw new
     * DatabaseException("String tables cannot be null or empty in SELECT query.");
     * 
     * if (tables.contains(",")) { String[] temp = tables.split(",");
     * 
     * for (String t : temp) if (t.split("`", -1).length - 1 == 2) throw new
     * DatabaseException("Invalid syntax in SELECT query. Two backticks must surround table " + t + ".");
     * 
     * this.tables = Arrays.asList(temp); } else { if (tables.split("`", -1).length - 1 == 2) throw new
     * DatabaseException("Invalid syntax in SELECT query. Two backticks must surround table " + tables + ".");
     * 
