    ...
    */
    ...
    public final void globalchat(final CommandContext args, final CommandSender sender) throws CommandException {
        ...
        if (player.hasPermission("simplechat.gobal.off")) {
            ...
            sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
                    "&6Globalen Chat betreten, tippe &4/globalchat&6 erneut um den Globalen Chat zu verlassen!"));
        } else {
            plugin.getPermission().playerAdd(player, "simplechat.gobal.off");
            sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
                    "&6Globalen Chat verlassen, tippe &4/globalchat&6 erneut um wieder im Globalen Chat lesen und schreiben zu dürfen!"));
        }
    }
    ...
    public final void rep(final CommandContext args, final CommandSender sender) throws CommandException {
        if (plugin.getChatter(sender.getName()).isLastWhisper()) {
            AsyncPlayerChatEvent chatEvent = new AsyncPlayerChatEvent(false, Bukkit.getPlayer(sender.getName()), "@"
                    + plugin.getChatter(sender.getName()).getLastWhisperFrom() + " " + args.getJoinedStrings(0), new HashSet<Player>(
    ...
    public final void w(final CommandContext args, final CommandSender sender) throws CommandException {
        if (args.argsLength() == 0) {
            sender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&6Chatpartner ("
                    + plugin.getChatter(sender.getName()).getConversionPartner() + ") gelöscht"));
        ...
        if (args.argsLength() == 1 && !plugin.getChatter(sender.getName()).isConversion()) {
            ...
            if (cPlayer == null) {
                sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
                        "&4Fehler beim Ermitteln des Chat Partners. Offline oder Verschrieben? (" + args.getString(0) + ")"));
                return;
            } else {
        ...
        if (plugin.getChatter(sender.getName()).isConversion()) {
            AsyncPlayerChatEvent chatEvent = new AsyncPlayerChatEvent(false, Bukkit.getPlayer(sender.getName()), args.getJoinedStrings(0),
                    new HashSet<Player>(Arrays.asList(Bukkit.getOnlinePlayers())));
            chatEvent.setFormat("%1$s -> %2$s");
            Bukkit.getServer().getPluginManager().callEvent(chatEvent);
        } else {
            AsyncPlayerChatEvent chatEvent = new AsyncPlayerChatEvent(false, Bukkit.getPlayer(sender.getName()), "@" + args.getString(0) + " "
                    + args.getJoinedStrings(1), new HashSet<Player>(Arrays.asList(Bukkit.getOnlinePlayers())));
            chatEvent.setFormat("%1$s -> %2$s");
            Bukkit.getServer().getPluginManager().callEvent(chatEvent);
        }
    }
